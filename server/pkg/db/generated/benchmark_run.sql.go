...
func (q *Queries) GetBenchmarkSuiteByWorkspaceAndSlug(ctx context.Context, arg GetBenchmarkSuiteByWorkspaceAndSlugParams) (BenchmarkSuite, error) {
	row := q.db.QueryRow(ctx, getBenchmarkSuiteByWorkspaceAndSlug, arg.WorkspaceID, arg.Slug)
...
const listCompleteRunsBySuiteSlug = `-- name: ListCompleteRunsBySuiteSlug :many
SELECT r.id, r.workspace_id, r.suite_id, r.suite_instance_ids, r.profile_id, r.base_run_id, r.display_name, r.status, r.status_reason, r.notes, r.evaluator_mode, r.adapter_version, r.submission_timeout_seconds, r.created_at, r.created_by, r.started_at, r.completed_at FROM benchmark_run r
...
type ListCompleteRunsBySuiteSlugParams struct {
	WorkspaceID pgtype.UUID `json:"workspace_id"`
	Slug        string      `json:"slug"`
	Limit       int32       `json:"limit"`
}

func (q *Queries) ListCompleteRunsBySuiteSlug(ctx context.Context, arg ListCompleteRunsBySuiteSlugParams) ([]BenchmarkRun, error) {
	rows, err := q.db.Query(ctx, listCompleteRunsBySuiteSlug, arg.WorkspaceID, arg.Slug, arg.Limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
