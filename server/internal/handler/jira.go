...
func (h *Handler) DeleteJiraConnection(w http.ResponseWriter, r *http.Request) {
	...
	if _, err := qtx.DeleteJiraCommentLinksByConnection(r.Context(), conn.ID); err != nil {
		writeError(w, http.StatusInternalServerError, "failed to delete jira comment links")
		return
	}
