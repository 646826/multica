...
type Handler struct {
	...
	// Jira integration (native sync). Always non-nil once wired in
	// cmd/server/router.go; Configured() is false when the master key
	// (MULTICA_JIRA_SECRET_KEY) is unset, and write endpoints return 503.
	Jira *jira.Service
